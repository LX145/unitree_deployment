...
{
    // Build a colour-mapped image: [-0.5,0.5] -> [0,1] -> grayscale -> colormap
    cv::Mat gray(h, w, CV_32FC1);
    for (int y = 0; y < h; ++y) {
        for (int x = 0; x < w; ++x) {
            float v = depth_obs[y * w + x];
            gray.at<float>(y, x) = (0.5f - v) / 1.0f;  // 0=far(2m)  1=near(0m)
        }
    }
    // far=2.0m → black (0),  near=0.0m → white (255)
    ...
    // Overlay text (white)
    char buf[128];
    snprintf(buf, sizeof(buf), "#%d  %dx%d  [black=far(2m) white=near(0m)]",
             frame_count, w, h);
